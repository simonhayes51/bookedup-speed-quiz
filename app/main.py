...
def require_role(request: Request, role: str | None = None):
...
def export_scores(room_id: str):
    ...
    return Response(out.getvalue().encode("utf-8"), media_type="text/csv", headers={"Content-Disposition": f"attachment; filename={room_id}_scores.csv"})
...
async def ws(ws: WebSocket):
    ...
    try:
        ...
        from .room_manager import Answer
        if role == "host":
            ...
            tid = f"t{int(time.time()*1000)%100000}_{len(room.teams)+1}"
            room.teams[tid] = Team(id=tid, name=team_name, score=0)
            room.team_connections[tid] = ws
            await manager.broadcast(room, {"type":"teams:update","teams":[{"id":t.id,"name":t.name,"score":t.score} for t in room.teams.values()]})
            await ws.send_json({"type":"team:joined","teamId":tid,"roomId":room.id})
        elif role == "display":
            if not room_id: await ws.send_json({"type":"error","message":"missing roomId"}); await ws.close(); return
            room = manager.get_room(room_id)
            if not room: await ws.send_json({"type":"error","message":"room not found"}); await ws.close(); return
        ...
        while True:
            ...
            if t == "host:set_quiz":
                ...
                tid = None
                for ttid, conn in room.team_connections.items():
                    if conn is ws: tid = ttid; break
                if not tid: continue
                bucket = room.answers.setdefault(q.id, {})
                if tid in bucket: await ws.send_json({"type":"answer:rejected","reason":"already answered"}); continue
                remaining = max(0, room.question_end_at - now)
                bucket[tid] = Answer(team_id=tid, question_id=q.id, option=int(data["option"]), submitted_at=now, ms_remaining=remaining)
...
# --- Admin minimal, plus Quiz Builder UI ---
@app.get("/admin/login")
...
def do_login(request: Request, email: str = Form(...), password: str = Form(...), next: str = Form("/admin")):
    row = conn.cursor().execute("SELECT id, email, name, password_hash, role FROM users WHERE email=?", (email,)).fetchone()
...
def logout(request: Request):
    request.session.clear(); return RedirectResponse("/admin/login", status_code=302)

@app.get("/admin")
def admin_home(request: Request):
    user = current_user(request)
    if not user: return RedirectResponse("/admin/login", status_code=302)
...
def hosts_add(request: Request, name: str = Form(...), email: str = Form(...), password: str = Form(...)):
    user = require_role(request, "admin")
    if not user: return RedirectResponse("/admin/login", status_code=302)
    conn.cursor().execute("INSERT INTO users(name,email,password_hash,role) VALUES(?,?,?,?)",(name,email,bcrypt.hash(password),"host")); conn.commit()
...
def venues_add(request: Request, name: str = Form(...), logo_url: str = Form("")):
    user = require_role(request, "admin")
    if not user: return RedirectResponse("/admin/login", status_code=302)
    conn.cursor().execute("INSERT INTO venues(name, logo_url) VALUES(?,?)",(name, logo_url)); conn.commit()
...
def venues_assign(request: Request, host_id: int = Form(...), venue_id: int = Form(...)):
    user = require_role(request, "admin")
    if not user: return RedirectResponse("/admin/login", status_code=302)
    conn.cursor().execute("INSERT OR IGNORE INTO hosts_venues(host_id, venue_id) VALUES(?,?)",(host_id, venue_id)); conn.commit()
    return RedirectResponse("/admin/venues", status_code=302)
...
def quiz_edit(request: Request, qid: int):
    ...
    if not row: return HTMLResponse("Not found", status_code=404)
